...
class PipelineAModel(nn.Module):
    ...
    def generate(self, pixel_values, input_ids, attention_mask):
        ...
        return self.qwen.generate(inputs_embeds=combined, attention_mask=combined_mask, max_new_tokens=50, do_sample=False, repetition_penalty=1.3)
...
image    = Image.open("/home/rs/24CS91R08/kathir/test_image.jpg").convert("RGB")
...
print("\n=== Pipeline A v2 (with Gemma) ===")
model_a = PipelineAModel().to(DEVICE)
s1 = torch.load("/home/rs/24CS91R08/kathir/checkpoints/stage1/stage1_best.pt", map_location="cpu")
s2 = torch.load("/home/rs/24CS91R08/kathir/checkpoints/stage2_v2/stage2_best.pt", map_location="cpu")
model_a.proj1.load_state_dict(s1["proj1"])
model_a.proj2.load_state_dict(s2["proj2"])
model_a.qwen.load_state_dict(s2["qwen"])
model_a.eval()
print("Answer:", infer(model_a, clip_processor, tokenizer, image, question))
del model_a
torch.cuda.empty_cache()

print("\n=== Pipeline B v2 (no Gemma) ===")
model_b = PipelineBModel().to(DEVICE)
s1 = torch.load("/home/rs/24CS91R08/kathir/checkpoints/stage1_nogemma/stage1_nogemma_best.pt", map_location="cpu")
s2 = torch.load("/home/rs/24CS91R08/kathir/checkpoints/stage2_nogemma_v2/stage2_nogemma_best.pt", map_location="cpu")
model_b.proj1.load_state_dict(s2["proj1"])
model_b.qwen.load_state_dict(s2["qwen"])
model_b.eval()
print("Answer:", infer(model_b, clip_processor, tokenizer, image, question))
